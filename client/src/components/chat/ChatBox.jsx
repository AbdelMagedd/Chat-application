...
const ChatBox = () => {
    ...
    const { currentChat ,isMessagesLoading ,sendMessage , messages} = useContext(ChatContext)
    ...
    return ( 
        <Stack gap={4} className="chat-box">
            ...
            <Stack gap={3} className="messages" >
                {messages && messages.map((message , index) => <Stack ref={scroll} key={index} className={`${message?.senderId === user._id ? "message self align-self-end flex-grow-0" : "message align-self-start flex-grow-0"}`}>
